...
function serve($conn, $date, $shift)
{
    ...
    $analisa = [];
    $stmt = $conn->prepare("
        SELECT id, nomor_antrian, register, brix_core, pol_core, pol_baca_core, rendemen_core
...
foreach ($dataRows as $row) {
    if ($row['brix_core'] !== null) {
        $avg['brix_core'] += $row['brix_core'];
        $avg['pol_core'] += $row['pol_core'];
        $avg['pol_baca_core'] += $row['pol_baca_core'];
        $avg['rendemen_core'] += $row['rendemen_core'];
        $avg['count']++;
    }
}

if ($avg['count'] > 0) {
    $avg['brix_core'] /= $avg['count'];
    $avg['pol_core'] /= $avg['count'];
    $avg['pol_baca_core'] /= $avg['count'];
    $avg['rendemen_core'] /= $avg['count'];
}

if ($_POST['handling'] == 'export') {
    header('Content-type: application/vnd-ms-excel');
    header('Content-Disposition: attachment; filename=Laporan_Core_Sample_' . $_POST["date"] . '.xls');
}
    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0 fw-semibold">
                ...
                Laporan Core Sample - <?= date('d/m/Y', strtotime($_POST['date'])) ?>
            </h5>
                                ...
                                <?php else: ?>
                                    <?php if (!empty($dataRows)): ?>
                                        <tr class="table-warning text-dark fw-semibold">
                                            ...
                                            <td><?= number_format($avg['brix_core'], 2, ',', '.') ?></td>
                                            <td><?= number_format($avg['pol_core'], 2, ',', '.') ?></td>
                                            <td><?= number_format($avg['pol_baca_core'], 2, ',', '.') ?></td>
                                            <td><?= number_format($avg['rendemen_core'], 2, ',', '.') ?></td>
                                        </tr>
                                    <?php endif; ?>
